...
class Pix2Pix_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ignore_skip=False, skip_weight=1.0,ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4
        ...
        super().__init__()
        self.use_pipeline_loader = use_pipeline_loader
        if use_pipeline_loader:
            ...
            print(f"Scheduler type: {type(self.sched)}")
            self.sched.set_timesteps(1, device="cuda")
            self.timesteps = self.sched.timesteps[0]
            print(f"Scheduler set with timesteps: {self.timesteps}")
            ...
            print("Loaded scheduler, UNet and VAE separately")
        ...
        if use_pipeline_loader is False:
    ...
    def forward(self, c_t, prompt=None, prompt_tokens=None, deterministic=True, r=1.0, noise_map=None):
        ...
        if deterministic:
            ...
            if self.use_pipeline_loader is False: 
                unet_input = encoded_control * r + noise_map * (1 - r)
            else:
                unet_input = noise_map
            ...
            latents = unet_input
            # Match diffusers: scale ONCE by init_noise_sigma
            if self.use_pipeline_loader is True:
                print("Scaling latents by init_noise_sigma from pipeline scheduler")
            ...
            if self.use_pipeline_loader is True:
                latent_model_input = self.sched.scale_model_input(latent_model_input, t)
            # runs the unet forward
            unet_output = self.unet(latent_model_input, self.timesteps, encoder_hidden_states=caption_enc,).sample
            self.unet.conv_in.r = None
            # Denoising step
            x_denoised = self.sched.step(unet_output, self.timesteps, latents, return_dict=True).prev_sample
