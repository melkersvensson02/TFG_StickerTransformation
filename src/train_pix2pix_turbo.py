...
def main(args):
    ...
    write_text(
        TEXT_FILE,
        f"net_pix2pix created; requires_grad counts: total_params={sum(p.numel() for p in net_pix2pix.parameters())}, trainable_params={sum(p.numel() for p in net_pix2pix.parameters() if p.requires_grad)}",
    )
    ...
    if accelerator.is_main_process and args.track_val_fid:
        ...
        write_text(
            TEXT_FILE,
            f"REF_STATS: mu shape={ref_stats[0].shape}, sigma shape={ref_stats[1].shape}\n",
        )
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            if args.print_pure_model:
                outf = os.path.join(
                    args.output_dir, "checkpoints", f"untouched_naked_model.pkl"
                )
            ...
            with accelerator.accumulate(*l_acc):
                ...
                with torch.no_grad():
                    ...
                    ink_mask = (
                        gray < 0.5
                    ).float()  # in the range, -1 is completly black and 1 is white
                    w_bg, w_fg = 0.5, 2.0
                    weights = (
                        w_bg + (w_fg - w_bg) * ink_mask
                    )  # 0.5 for bg, 2.0 for lines (all black dots at 1 and white are 0 in the mask)
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 1 and global_step != 1:
                        outf = os.path.join(
                            args.output_dir,
                            "checkpoints",
                            f"model_model_Fill50k_{global_step}.pkl",
                        )
                        accelerator.unwrap_model(net_pix2pix).save_model(outf)
                    ...
                    if global_step % args.eval_freq == 1:
                        ...
                        if len(train_loss_buffer["l2"]) > 0:
                            avg_train_l2 = sum(train_loss_buffer["l2"]) / len(
                                train_loss_buffer["l2"]
                            )
                            avg_train_lpips = sum(train_loss_buffer["lpips"]) / len(
                                train_loss_buffer["lpips"]
                            )
                            avg_train_G = sum(train_loss_buffer["G"]) / len(
                                train_loss_buffer["G"]
                            )
                            avg_train_D = sum(train_loss_buffer["D"]) / len(
                                train_loss_buffer["D"]
                            )
                            if args.lambda_clipsim > 0:
                                avg_train_clipsim = sum(
...
def save_debug_batch(batch, outdir, prefix, max_images=8):
    ...
    x_tgt = batch[
        "output_pixel_values"
    ]  # [B, C, H, W], in [-1, 1] because of normalize(mean=0.5, std=0.5)
