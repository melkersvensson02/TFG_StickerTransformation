...
from cyclegan_turbo_resume import CycleGAN_Turbo, VAE_encode, VAE_decode, initialize_unet, initialize_vae, load_checkpoint_into_models
...
def main(args):
    ...
    if args.resume_from_checkpoint:
        if os.path.isfile(args.resume_from_checkpoint):
            ...
            # Load checkpoint into models
            sd = load_checkpoint_into_models(checkpoint_path, unet, vae_a2b, vae_b2a)
            
            # Extract step number from checkpoint filename
            # Expected format: model_XXXX.pkl
            basename = os.path.basename(checkpoint_path)
            if "model_" in basename:
                try:
                    resume_step = int(basename.replace("model_", "").replace(".pkl", ""))
                    write_text(TEXT_FILE, f"Resuming from step: {resume_step}\n")
                except:
                    write_text(TEXT_FILE, f"Could not parse step from filename, starting from 0\n")
        else:
            write_text(TEXT_FILE, f"Checkpoint file not found: {args.resume_from_checkpoint}\n")
            write_text(TEXT_FILE, "Starting training from scratch\n")
    # ===== END RESUME LOGIC =====
    ...
    first_epoch = 0
    global_step = resume_step  # Start from resume step instead of 0
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                """
                Discriminator for domain B (receives input from domain A translated to B)
                """
                loss_D_B_real = net_disc_a(img_b, for_real=True).mean() * args.lambda_gan
                loss_D_B_fake = net_disc_a(fake_b.detach(), for_real=False).mean() * args.lambda_gan
                loss_D_B = (loss_D_B_real + loss_D_B_fake) / 2
                accelerator.backward(loss_D_B, retain_graph=False)
                if accelerator.sync_gradients:
                ...
                """
                Discriminator for domain A (receives input from domain B translated to A)
                """
                loss_D_A_real = net_disc_b(img_a, for_real=True).mean() * args.lambda_gan
                loss_D_A_fake = net_disc_b(fake_a.detach(), for_real=False).mean() * args.lambda_gan
                loss_D_A = (loss_D_A_real + loss_D_A_fake) / 2
                accelerator.backward(loss_D_A, retain_graph=False)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_disc, args.max_grad_norm)
                optimizer_disc.step()
                lr_scheduler_disc.step()
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 1:
                        ...
                        torch.save(sd, outf)
                        write_text(TEXT_FILE, f"\nCheckpoint saved at step {global_step}: {outf}\n")
                        gc.collect()
                    ...
                    if global_step % args.validation_steps == 1:
                        ...
                        # Create logs dict
                        logs = {}
                        logs["loss_cycle_a"] = loss_cycle_a.detach().item()
                        logs["loss_cycle_b"] = loss_cycle_b.detach().item()
                        logs["loss_gan_a"] = loss_gan_a.detach().item()
                        logs["loss_gan_b"] = loss_gan_b.detach().item()
                        logs["loss_D_A"] = loss_D_A.detach().item()
                        logs["loss_D_B"] = loss_D_B.detach().item()
...
Command to RESUME training from step 4001:
export NCCL_P2P_DISABLE=1
accelerate launch src/Cyclegan_Train_turbo_resume.py \
    --pretrained_model_name_or_path="/data/upftfg19/mfsvensson/TFG_weights/img2img-turbo" \
    --output_dir="outputs/myUnPairedDataset/" \
    ...
    --dataloader_num_workers 7 \
    --mixed_precision bf16 \
    --resume_from_checkpoint="outputs/myUnPairedDataset/checkpoints/model_4001.pkl"
