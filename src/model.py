...
def my_vae_decoder_fwd(self, sample, latent_embeds=None):
    ...
    if not self.ignore_skip:
        ...
        for idx, up_block in enumerate(self.up_blocks):
            ...
            # Ensure size are the same 
            if sample.shape != skip_in.shape:
                min_h = min(sample.shape[2], skip_in.shape[2])
                min_w = min(sample.shape[3], skip_in.shape[3])
                sample = sample[:, :, :min_h, :min_w]
                skip_in = skip_in[:, :, :min_h, :min_w]
            # add skip
            sample = sample + skip_in * self.skip_weight
