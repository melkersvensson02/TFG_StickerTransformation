...
def parse_args_paired_training(input_args=None):
   ...
   """
    ...
    parser.add_argument("--ignore_skip", default=False,
                    help="Disable VAE encoder->decoder skip connections.")
    parser.add_argument("--skip_weight", type=float, default=1.0,
                        help="Strength of VAE skip additions (only used if not ignore_skip).")
    parser.add_argument("--use_canny_conditioning", default=False,
                    help="Use canny edges as conditioning instead of RGB.")
    ...
    # Optional: if you want a hard binary mask instead of Canny:
    ...
    parser.add_argument(
        ...
        help=(
            "Which paired dataset format to use. "
            "'original' = paper setup with per-image prompts via *_prompts.json. "
            "'modified' = single global prompt and automatic PNG conversion."
        ),
    )
...
def convert_folder_to_png(folder_path):
    for filename in os.listdir(folder_path):
        if filename.lower().endswith(('.jpg', '.jpeg')):
            ...
            img.save(os.path.join(folder_path, png_name), 'PNG')
            os.remove(img_path)  # Remove old jpg
            print(f"Converted {filename} to {png_name}")
...
class PairedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        ...
        if split == "train":
            ...
            # Convert all images in the training folders to PNG
            convert_folder_to_png(self.input_folder)
            convert_folder_to_png(self.output_folder)
        elif split == "test":
            self.input_folder = os.path.join(dataset_folder, "test_A")
            self.output_folder = os.path.join(dataset_folder, "test_B")
            # Convert all images in the test folders to PNG
            convert_folder_to_png(self.input_folder)
            convert_folder_to_png(self.output_folder)
        
        # Get all image names from input folder
        self.img_names = [f for f in os.listdir(self.input_folder) if f.endswith(('.png', '.jpg', '.jpeg'))]
        self.img_names.sort()
        
        # Single prompt for all images
        self.caption = "Generate a minimalist, binary (black and white) image of main subjects.Style: tattoo-inspired, focusing on the contour and the most relevant details.Use bold, thick, clean black lines only. No interior fill. Make the output suitable for vinyl cutting. Center the design on a pure white background. Optimize for vinyl cutting. Make facial features natural and simplified. Avoid: thin or sketchy lines, shading, gradients, colors, soft edges, background elements, and noise. No output image borders nor frames."
    ...
    def __getitem__(self, idx):
        ...
        caption = self.caption  # Use the single caption
        ...
        # Resize both input and output to the same divisible-by-8 size
...
#Info about original code get_items:
"""
...
class PairedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer, data_set_type="original", 
                use_canny_conditioning=False, canny_low_threshold=100, canny_high_threshold=200,
                binary_threshold=None):
        """
        Initialize the paired dataset object.

        data_set_type:
            - "original": uses per-image captions from train_prompts.json / test_prompts.json
                          (the paper's original repo behavior).
            - "modified": uses a single global caption for all images and converts JPG→PNG.
        """
        super().__init__()
        ...
        if split == "train":
            self.input_folder = os.path.join(dataset_folder, "train_A")
            self.output_folder = os.path.join(dataset_folder, "train_B")
            self.captions_path = os.path.join(dataset_folder, "train_prompts.json")
        else:  # split == "test"
            self.input_folder = os.path.join(dataset_folder, "test_A")
            self.output_folder = os.path.join(dataset_folder, "test_B")
            self.captions_path = os.path.join(dataset_folder, "test_prompts.json")

        if self.data_set_type == "original":
            # === Paper's behavior: per-image prompts from JSON ===
            with open(self.captions_path, "r") as f:
                self.captions = json.load(f)
            self.img_names = list(self.captions.keys())
        else:
            # === Your modified behavior: single global caption + PNG conversion ===
            # Convert all images in the folders to PNG (if needed)
            convert_folder_to_png(self.input_folder)
            convert_folder_to_png(self.output_folder)
    ...
    def __getitem__(self, idx):
        ...
        input_img = Image.open(os.path.join(self.input_folder, img_name))
        output_img = Image.open(os.path.join(self.output_folder, img_name))
        # Your safer handling for arbitrary sources
        if input_img.mode == "RGBA":
            input_img = input_img.convert("RGB")
        if output_img.mode == "RGBA":
            output_img = output_img.convert("RGB")
        if self.data_set_type == "modified":
            # Enforce divisible-by-8 spatial size (important for VAE / UNet)
            W, H = input_img.size
            new_w = (W // 8) * 8
            new_h = (H // 8) * 8
            input_img = input_img.resize((new_w, new_h), Image.LANCZOS)
            output_img = output_img.resize((new_w, new_h), Image.LANCZOS)
            caption = self.caption  # single global caption

        else:
            # Original paper behavior: no extra resizing logic, per-image caption
        ...
        img_t = F.to_tensor(img_t)
        """
        img_in = self.T(input_img)

        if self.use_canny_conditioning:
            if self.binary_threshold is not None:
                # We know the range to be this becase F.to_tensor() guarantes it for 
                # if the input is a PIL image or NumPy array with dtype uint8
                t = F.to_tensor(img_in)  # [0,1]
                
                # Bright → white, dark → black
                img_t = (t.mean(dim=0, keepdim=True) > self.binary_threshold).float()
                ...
                # Invert Canny: white background, black edges
                img_t = 1.0 - F.to_tensor(canny)

        else:
            img_t = F.to_tensor(img_in)
...
class UnpairedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        ...
        if split == "train":
            ...
            # Convert all images in the training folders to PNG
            convert_folder_to_png(self.source_folder)
            convert_folder_to_png(self.target_folder)
        elif split == "test":
    ...
    def __getitem__(self, index):
        ...
        For the source domain, if the ested index is within the range of available images,
        ...
        # Enforce divisible by 8
        W, H = img_pil_src.size
        new_w = (W // 8) * 8
        new_h = (H // 8) * 8
        img_pil_src = img_pil_src.resize((new_w, new_h), Image.LANCZOS)
        img_pil_tgt = img_pil_tgt.resize((new_w, new_h), Image.LANCZOS)
