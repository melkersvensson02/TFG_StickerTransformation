...
def main(args):
    ...
    print_gpu_info("after model init")
    ...
    print_gpu_info("after accelerator.prepare")
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step == 0:
                print_gpu_info("before first forward")
            with accelerator.accumulate(*l_acc):
                ...
                print_gpu_info("after optimizer step")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    print("Running initial FID and DINO (debug mode)")
                    ...
                    if global_step == 1:
                        print_gpu_info("before doing FID")
                        ...
                        compute FID for "B->A"
                        """
                        print_gpu_info("compute FID for B->A now")
                        ...
                        print_gpu_info("after initial FID+DINO")
                        # Save the parameters 
...
def print_gpu_info(tag=""):
    if not torch.cuda.is_available():
        print(f"[{tag}] CUDA not available", file=GPU_LOG_FILE or sys.stdout)
        return
    ...
    print(
        ...
        file=GPU_LOG_FILE or sys.stdout
    )
...
if __name__ == "__main__":
    ...
    GPU_LOG_FILE = open(os.path.join(args.output_dir, "gpu_log.txt"), "a")
    main(args)
