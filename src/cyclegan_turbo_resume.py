...
def load_checkpoint_into_models(checkpoint_path, unet, vae_a2b, vae_b2a):
    """
    Load a checkpoint into existing UNet and VAE models.
    Returns the loaded state dict for reference.
    """
    ...
    # Load VAE weights for both directions
    print("Loading VAE weights...")
    # The checkpoint contains full state dicts for vae_enc and vae_dec wrappers
    # We need to extract and load into vae_a2b and vae_b2a appropriately
    
    # Load VAE encoder weights (both directions stored in sd_vae_enc)
    vae_enc_temp = VAE_encode(vae_a2b, vae_b2a=vae_b2a)
    vae_enc_temp.load_state_dict(sd["sd_vae_enc"])
    
    # Load VAE decoder weights (both directions stored in sd_vae_dec)
    vae_dec_temp = VAE_decode(vae_a2b, vae_b2a=vae_b2a)
    vae_dec_temp.load_state_dict(sd["sd_vae_dec"])
    
    print("Checkpoint loaded successfully!")
    
    # Verify skip layers are loaded correctly
    print("\n=== VAE Skip Layers Verification ===")
    for name, param in vae_a2b.named_parameters():
        if 'skip_conv' in name.lower():
            param_sum = param.abs().sum().item()
            param_mean = param.abs().mean().item()
            print(f"VAE_A2B {name}: sum={param_sum:.6f}, mean={param_mean:.6f}")
            if param_sum < 1e-6:
                print(f"  WARNING: {name} appears to be all zeros!")
    
    for name, param in vae_b2a.named_parameters():
        if 'skip_conv' in name.lower():
            param_sum = param.abs().sum().item()
            param_mean = param.abs().mean().item()
            print(f"VAE_B2A {name}: sum={param_sum:.6f}, mean={param_mean:.6f}")
            if param_sum < 1e-6:
                print(f"  WARNING: {name} appears to be all zeros!")
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        super().__init__()
        self.tokenizer = AutoTokenizer.from_pretrained("/data/upftfg19/mfsvensson/TFG_weights/img2img-turbo", subfolder="tokenizer")
        ...
        if pretrained_name == "day_to_night":
            url = "./checkpoints/day2night.pkl"
            self.load_ckpt_from_local(url)
            self.timesteps = torch.tensor([999], device="cuda").long()
            self.caption = "driving in the night"
            self.direction = "a2b"
        elif pretrained_name == "night_to_day":
            url = "./checkpoints/night2day.pkl"
            self.load_ckpt_from_local(url, ckpt_folder)
            self.timesteps = torch.tensor([999], device="cuda").long()
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        # Verify skip_layers are not all zeros
        print("\n=== VAE Skip Layers Verification ===")
        for name, param in self.vae_enc.named_parameters():
            if 'skip_conv' in name.lower():
                param_sum = param.abs().sum().item()
                param_mean = param.abs().mean().item()
                print(f"VAE_ENC {name}: sum={param_sum:.6f}, mean={param_mean:.6f}")
                if param_sum < 1e-6:
                    print(f"  WARNING: {name} appears to be all zeros!")

        for name, param in self.vae_dec.named_parameters():
            if 'skip_conv' in name.lower():
                param_sum = param.abs().sum().item()
                param_mean = param.abs().mean().item()
                print(f"VAE_DEC {name}: sum={param_sum:.6f}, mean={param_mean:.6f}")
                if param_sum < 1e-6:
                    print(f"  WARNING: {name} appears to be all zeros!")

    def load_ckpt_from_local(self, local_path):
        sd = torch.load(local_path)
        self.load_ckpt_from_state_dict(sd)
