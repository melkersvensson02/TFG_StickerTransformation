...
def pil_to_tensor(img: Image.Image) -> torch.Tensor:
    """[-1, 1] — Pix2Pix_Turbo is trained with this range."""
    x = F.to_tensor(img)
...
def assemble_row(panels: list, group_sizes: list):
    ...
    between groups.  group_sizes = [1, n_sem, n_nonsem] (ref always 1).
...
def build_group_header(total_w: int, xs: list, widths: list,
    ...
    group_sizes = [1, n_sem, n_nonsem]  (ref group first, no label drawn for it)
...
@torch.no_grad()
def run_model(model: Pix2Pix_Turbo, img: Image.Image, prompt: str) -> Image.Image:
    """Pix2Pix_Turbo: expects [-1, 1] input, returns [-1, 1] output."""
    out = model(pil_to_tensor(img), prompt=prompt)
    return tensor_to_pil(out)


def load_model(path: str, use_fp16: bool) -> Pix2Pix_Turbo:
    m = Pix2Pix_Turbo(pretrained_path=path)
    m.set_eval()
...
def parse_args():
    ap = argparse.ArgumentParser(description="Multi-model ablation comparison grid (pix2pix_turbo).")
...
def main():
    ...
    n_sem, n_nonsem = len(sem_cfgs), len(nonsem_cfgs)

    if n_sem + n_nonsem == 0:
        raise ValueError("No models found in config JSON.")

    print(f"Loading {n_sem} semantic + {n_nonsem} non-semantic models …")
    sem_models    = [(c["name"], load_model(c["path"], args.use_fp16)) for c in sem_cfgs]
    nonsem_models = [(c["name"], load_model(c["path"], args.use_fp16)) for c in nonsem_cfgs]
    ...
    NSEM_LBG = (75, 28, 28)    # red tint   — non-semantic
    ...
    for fname in image_names:
        ...
        sem_out_orig    = [(n, run_model(m, img_orig,   prompt))       for n, m in sem_models]
        nsem_out_masked = [(n, run_model(m, img_masked, fixed_prompt)) for n, m in nonsem_models]
        nsem_out_orig   = [(n, run_model(m, img_orig,   fixed_prompt)) for n, m in nonsem_models]

        # ── build panel rows ──────────────────────────────────────────────────
        def make_panels(ref_img, ref_lbl, sem_pairs, nsem_pairs):
        ...
        panels_masked = make_panels(img_masked, "Ref (masked)",   sem_out_masked, nsem_out_masked)
        panels_orig   = make_panels(img_orig,   "Ref (original)", sem_out_orig,   nsem_out_orig)
